...
def uniformCostSearch(problem):
    ...
    action = []
    startState = problem.getStartState()
    pQueue.push((startState, action), 0)

    while not pQueue.isEmpty():
        currentState, action = pQueue.pop()

        if (currentState not in closeList):
            closeList.append(currentState)

            if (problem.isGoalState(currentState)):
                return action

            for succesor in problem.getSuccessors(currentState):
                if succesor[0] not in closeList:
                    pQueue.push((succesor[0], action + [succesor[1]]),problem.getCostOfActions(action + [succesor[1]]))
        else:
            continue
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    action = []
    startState = problem.getStartState()
    pQueue.push((startState,action),heuristic(startState, problem))
    ...
    while not pQueue.isEmpty():
        ...
        if(currentState not in closeList):
            ...
            for succesor in problem.getSuccessors(currentState):
                if succesor[0] not in closeList:
                    pQueue.push((succesor[0], action + [succesor[1]]),problem.getCostOfActions(action + [succesor[1]]) + heuristic(succesor[0], problem))
        else:
            continue
    util.raiseNotDefined()
