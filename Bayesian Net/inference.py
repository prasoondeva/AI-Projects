    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            ...
            for newPosition, posProb in newPositionDist.items():
                futureBeliefs[newPosition] += posProb * self.beliefs[oldPosition]
        self.beliefs = futureBeliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(legals[i % len(legals)])
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            newPositionDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldParticle))
            futureParticles.append(util.sample(newPositionDist))
        self.particles = futureParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(len(possibleGhostPositions)):
            self.particles.append(possibleGhostPositions[i % len(possibleGhostPositions)])
        ...
        for ghost in range(self.numGhosts):
            if noisyDistances[ghost] is None:
                for particle in range(self.numParticles):
                    self.particles[particle] = self.getParticleWithGhostInJail(self.particles[particle], ghost)
        beliefs = self.getBeliefDistribution()
        beliefTrueDistances = self.getBeliefDistribution()
        for ghost in range(self.numGhosts):
            if noisyDistances[ghost] is not None:
                for particle in beliefs:
                    ...
                    beliefTrueDistances[particle] *= emissionModels[ghost][trueDistance]
        if beliefTrueDistances.totalCount() == 0:
            self.initializeParticles()
        else:
            beliefTrueDistances.normalize()
            resampledParticles = []
            for x in range(0, self.numParticles):
                resampledParticles.append(util.sample(beliefTrueDistances))
            self.particles = resampledParticles
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                ...
                newPositionDist = getPositionDistributionForGhost(gameState, ghost, self.ghostAgents[ghost])
                newParticle[ghost] = util.sample(newPositionDist)
            "*** END YOUR CODE HERE ***"
