...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        currentLocalMaximum = set()
        for belief in livingGhostPositionDistributions:
            currentLocalMaximum.add(belief.argMax())
        bestAction = None
        bestActionProbability = 0
        for i, position in enumerate(currentLocalMaximum):
            if livingGhostPositionDistributions[i][position] >= bestActionProbability:
                bestAction = position
                bestActionProbability = livingGhostPositionDistributions[i][position]
        possibleActions = []
        for possibleAction in legal:
            nextPosition = Actions.getSuccessor(pacmanPosition, possibleAction)
            mazeDistance = self.distancer.getDistance(nextPosition, bestAction)
            possibleActions.append((mazeDistance, possibleAction))
        resultAction = min(possibleActions)
        return resultAction[1]